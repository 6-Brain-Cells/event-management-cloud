...
def _parse_dt(value: str) -> datetime:
    """
    Accepts common frontend datetime formats:
    - "YYYY-MM-DD HH:MM:SS"
    - "YYYY-MM-DDTHH:MM"
    - "YYYY-MM-DDTHH:MM:SS"
    """
    v = (value or "").strip()
    # datetime.fromisoformat supports both " " and "T" separators
    try:
        return datetime.fromisoformat(v)
    except ValueError:
        # Some browsers omit seconds in datetime-local; add ":00" if needed
        if "T" in v and len(v) == 16:  # "YYYY-MM-DDTHH:MM"
...
def create_event(event: EventCreate):
    ...
    try:
        ...
        """, (event.title, event.description, event.event_type,
              start_dt, end_dt, event.location,
